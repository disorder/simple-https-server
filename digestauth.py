# https://svn.python.org/projects/sandbox/trunk/digestauth/digestauth.py
...
def generate_nonce(bits, randomness=None):
    ...
    nonce = sha1((str(randomness) + str(time.time()) +
...
class DigestAuthServer:
    ...
    def get_algorithm_impls(self, algorithm=None):
        ...
        if algorithm == 'MD5':
            H = lambda x: md5(x.encode('utf-8')).hexdigest()
        elif algorithm == 'SHA':
            H = lambda x: sha1(x.encode('utf-8')).hexdigest()
        # XXX MD5-sess
